...
class SparkCluster:
    ...
    def scale(self, worker_num: int):
        logger.info('Spark cluster is scaling...')
        n = len(self.workers)
        threads: List[threading.Thread] = []
        if worker_num < n:
            ...
            for worker_id in range(n, worker_num):
                worker = SparkWorker(aws=self._aws, ec2_instance_type=self._worker_instance_type, worker_id=worker_id)
                worker.spark_master_private_ip = self.master.private_ip
        ...
        for thread in threads:
            thread.join()
        logger.info('Spark cluster has finished scaling.')
