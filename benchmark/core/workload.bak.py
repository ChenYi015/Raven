...
class WorkloadManager:
    ...
    def __init__(self):
        ...
        self.current_concurrency: atomics.INTEGRAL = atomics.atomic(width=16, atype=atomics.INT)
        self._thread_pool = ThreadPoolExecutor(
            max_workers=self.max_concurrency,
            thread_name_prefix='QueryGenerator'
        )
        self._generate_switch: bool = False

    def generate_queries_with_stable_concurrency(self, *,  concurrency: int, execute_queue: Queue):
        """生成具有固定并发度的查询."""
        if concurrency > self.max_concurrency:
            raise ValueError('Specified concurrency has exceeded max concurrency.')
