...
# Parse options
function usage() {
  echo "Usage: $(basename "$0"): --user <user> [--kylin-version <kylin-version>] --s3-path <s3-path>  [-h|--help]"
}

if ! TEMP=$(getopt -o h --long user:,kylin-version:,s3-path:,help -- "$@"); then
  usage
fi
...
if [ -f ${kylin_tarball} ]; then
  ...
  if ! aws s3 cp "${s3_path}"/tars/${kylin_tarball} .; then
